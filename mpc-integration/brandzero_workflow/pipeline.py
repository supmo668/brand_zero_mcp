...
async def analyze_results(
    ...
    if isinstance(result, BrandAnalysisResult):
        ...
        # Unpack dictionary result into BrandAnalysisResult
        analysis_result = BrandAnalysisResult(**result)
    else:
...
def ensure_brand_analysis_result(obj):
    if isinstance(obj, BrandAnalysisResult):
        return obj
    elif isinstance(obj, dict):
        return BrandAnalysisResult(**obj)
    else:
        raise ValueError(f"Cannot convert to BrandAnalysisResult: {type(obj)}")
...
async def run_analysis_pipeline(brand_or_product: str) -> BrandAnalysisResult:
    ...
    try:
        ...
        return ensure_brand_analysis_result(analysis_result)
